...
server.setNotFoundHandler(async (request, reply) => {
  if (request.url === '/' && request.method === 'GET') {
    reply.type('text/html')
    return reply.send(`<html><head><title>GlosX - Your voice, on every network</title></head><body style="font-family:sans-serif;max-width:600px;margin:50px auto;padding:20px"><h1>GlosX</h1><p>Transform your ideas into social media content for 10 platforms using AI.</p><p>Available on iOS and Android.</p><a href="/privacy">Privacy Policy</a> | <a href="/terms">Terms</a></body></html>`)
  }
  reply.status(404).send({ message: `Route ${request.method}:${request.url} not found`, error: 'Not Found', statusCode: 404 })
})
